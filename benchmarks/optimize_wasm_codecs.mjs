...
function wasmOptBin() {
  // npm on Windows exposes `wasm-opt.cmd`.
...
for (const file of wasmFiles) {
  ...
  if (res.error) {
    console.error(`${basename(file)}: failed to run wasm-opt: ${res.error.message}`);
    failed = true;
    continue;
  }
  if (res.status !== 0) {
  ...
  process.stdout.write(`${basename(file)}: ${before} -> ${after} bytes (${pct}% smaller)\n`);
}
