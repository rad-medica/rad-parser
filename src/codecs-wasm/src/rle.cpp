...
WASM_EXPORT int decode_rle(const uint8_t* data, size_t len, uint32_t width, uint32_t height, uint32_t components) {
    ...
    // For 16-bit single-component: 2 segments (MSB/LSB) but output is interleaved bytes
    // For other cases: num_segments == components
    size_t dest_len;
    if (num_segments == 2 && components == 1) {
        // 16-bit: 2 bytes per pixel
        dest_len = width * height * 2;
    } else {
        // Standard: num_segments == components
        dest_len = width * height * components;
    }
    ...
    if (num_segments == 2 && components == 1) {
        // Decode MSB segment (segment 0) to odd byte positions
        // Decode LSB segment (segment 1) to even byte positions
        size_t num_pixels = width * height;
        uint8_t* msb_buf = (uint8_t*)malloc(num_pixels);
        uint8_t* lsb_buf = (uint8_t*)malloc(num_pixels);
        if (!msb_buf || !lsb_buf) {
            if (msb_buf) free(msb_buf);
            if (lsb_buf) free(lsb_buf);
            free(dest);
            return -3;
        }
        ...
        for (uint32_t s = 0; s < 2; ++s) {
            ...
            uint8_t* seg_buf = (s == 0) ? msb_buf : lsb_buf;
            size_t src_idx = start;
            size_t dest_idx = 0;

            while (src_idx < end && dest_idx < num_pixels) {
                int8_t n = (int8_t)data[src_idx++];

                if (n >= 0) {
                    int count = n + 1;
                    for (int k = 0; k < count; ++k) {
                        if (src_idx >= end || dest_idx >= num_pixels) break;
                        seg_buf[dest_idx++] = data[src_idx++];
                    }
                } else if (n > -128) {
                    int count = -n + 1;
        ...
        for (size_t p = 0; p < num_pixels; ++p) {
            dest[p * 2] = lsb_buf[p];     // LSB at even offset
            dest[p * 2 + 1] = msb_buf[p]; // MSB at odd offset
        }
