...
export fn get_result_ptr() ?[*]u8 { return last_result_ptr; }
export fn get_result_len() usize { return last_result_len; }
...
const opj_image_comp_t = extern struct {
    dx: u32, dy: u32, w: u32, h: u32, x0: u32, y0: u32,
    prec: u32, bpp: u32, sgnd: u32, resno_decoded: u32, factor: u32,
    data: [*]i32,
    alpha: u16,
};
...
const opj_cparameters_t = extern struct {
    tile_size_on: bool,
    cp_tx0: c_int, cp_ty0: c_int, cp_tdx: c_int, cp_tdy: c_int,
    cp_disto_alloc: c_int, cp_fixed_alloc: c_int, cp_fixed_quality: c_int,
    cp_matrice: ?*c_int,
    cp_comment: ?*c_char,
    csty: c_int,
    prog_order: c_int,
...
export fn encode_jpeg(pixel_data: [*]const u8, len: usize, width: u32, height: u32, quality: u8) c_int {
    ...
    if (tjCompress2(handle, pixel_data, @as(c_int, @intCast(width)), 0, @as(c_int, @intCast(height)), TJPF_RGB, 
        &jpegBuf, &jpegSize, TJSAMP_444, @as(c_int, @intCast(quality)), TJFLAG_FASTDCT) != 0) return -2;
...
export fn encode_jpeg2000(pixel_data: [*]const u8, len: usize, width: u32, height: u32, bits_per_sample: u8, components: u8) c_int {
    ...
    for (0..@as(usize, components)) |i| {
        cmptparm[i].dx = 1; cmptparm[i].dy = 1;
        cmptparm[i].w = width; cmptparm[i].h = height;
        cmptparm[i].x0 = 0; cmptparm[i].y0 = 0;
