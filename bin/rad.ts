...
async function main() {
    ...
    const wasmCorePath = new URL('../src/wasm-core-build/rad_parser_wasm_core_bg.wasm', import.meta.url);
    const wasmCodecsPath = new URL('../src/wasm-codecs-build/rad_parser_wasm_codecs_bg.wasm', import.meta.url);
...
async function runTranscode(args: string[]) {
    ...
    if (args.length < 4) { // input output --format value
    ...
    for (let i = 2; i < args.length; i++) {
        if (args[i] === '--format' && i + 1 < args.length) {
            ...
            if (targetFormat === 'jpeg') targetFormat = '1.2.840.10008.1.2.4.50'; // Baseline
    ...
    const newDataset = await transcode(dataset, { targetTransferSyntax: targetFormat });
...
async function runImage(args: string[]) {
    ...
    if (args.length < 2) {
        console.log("Usage: rad image <input.dcm> <output.png> [--frame 0] [--format png]");
        return;
    }
...
async function runDump(args: string[]) {
    ...
    for (const tag of sortedTags) {
        ...
        if (el.Value instanceof Uint8Array) {
            ...
            valStr = el.Value.map(v => String(v)).join('\\').substring(0, 100);
