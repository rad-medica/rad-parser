...
describe('extractRescaledPixelData', () => {
  test('should extract and rescale pixel data with default parameters', () => {
    ...
    try {
      ...
      console.log(`Min: ${Math.min(...rescaled)}, Max: ${Math.max(...rescaled)}`);
    } catch (error) {
  ...
  test('should handle 16-bit signed pixel data', () => {
    ...
    try {
      ...
      console.log(`Pixel representation: ${pixelRepresentation} (0=unsigned, 1=signed)`);
  ...
  test('should throw error if pixel data is missing', () => {
    ...
    expect(() => extractRescaledPixelData(mockDataset)).toThrow('Pixel data not found');
  });
